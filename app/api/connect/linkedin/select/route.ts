...
/**
 * POST /api/connect/linkedin/select
 *
 * Consumes a pending OAuth session: saves selected LinkedIn ad accounts to the
...
export async function POST(request: Request) {
  ...
  if (fetchError || !session) {
    console.error('[linkedin/select] Session not found:', sessionId, fetchError?.message)
  ...
  if (updateError) {
    console.error('[linkedin/select] Failed to mark session consumed:', updateError)
  ...
  for (const account of toSave) {
    ...
    if (savedAccount) {
      try {
        ...
        console.error(`[linkedin/select] Initial pull failed for ${account.id}:`, err)
      }
    }
  }
